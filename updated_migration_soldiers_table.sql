...
-- Create new soldiers table with proper schema for base64 images
CREATE TABLE soldiers (
  ...
  photo_data TEXT, -- Changed to TEXT to handle large base64 strings
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON soldiers;
DROP POLICY IF EXISTS "Allow all operations for anonymous users" ON soldiers;
...
DROP TRIGGER IF EXISTS update_soldiers_updated_at ON soldiers;
